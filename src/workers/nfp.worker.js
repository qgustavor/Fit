...
import { noFitRectanglePolygon, noFitSimplifiedPolygon, noFitPolygon, minkowskiDifference } from '../util.js'
...
self.addEventListener('message', (e) => {
  ...
  let result
  if (e.data.A.isBin) {
    let polygon = e.data.A.isRectangle
      ? noFitRectanglePolygon(pa, pb, e.data.inside, e.data.edges)
      : noFitSimplifiedPolygon(pa, pb, e.data.inside, e.data.edges)
    if (polygon) {
      result = [polygon]
    } else {
      result = []
